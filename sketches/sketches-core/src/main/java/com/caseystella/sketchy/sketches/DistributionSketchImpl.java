...
/**
 * A (near) constant memory implementation of a statistics provider. For first order statistics, simple terms are stored
 * and composed to return the statistics results. This is intended to provide a mergeable implementation for a
 * statistics provider.
 */
public class DistributionSketchImpl<T extends Number, S> implements DistributionSketch<T> {
    ...
    /**
     * Add a value. NOTE: This does not store the point, but only updates internal state. NOTE: This is NOT threadsafe.
     * 
     * @param value
     */
    @Override
    ...
    private void checkFlowError(double sumOfSquares, double sum, double... vals) {
        ...
        // underflow. It is sufficient to check sumOfSquares because sumOfSquares is going to converge to 0 faster than
        // sum
    ...
    public DistributionSketch<T> merge(DistributionSketch<T> provider) {
        ...
        combined.M4 = a.M4 + b.M4
                + delta4 * a.n * b.n * (a.n * a.n - a.n * b.n + b.n * b.n) / (combined.n * combined.n * combined.n);
        combined.M4 += 6.0 * delta2 * (a.n * a.n * b.M2 + b.n * b.n * a.M2) / (combined.n * combined.n)
                + 4.0 * delta * (a.n * b.M3 - b.n * a.M3) / combined.n;

        // Merging the distributional sketches
        combined.sketch = sketchType.merge(a.sketch, b.sketch);
        // checkFlowError(combined.sumOfSquares, sum, combined.sumOfSquares, combined.M1, combined.M2, combined.M3,
        // combined.M4);
        return combined;
    }
