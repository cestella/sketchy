...
/**
 * An inclusion/exclusion specifier which matches against a specific date. The default format is
 * yyyy/MM/dd
 */
public class DateSpecifierPredicate implements Predicate<Long> {
  ...
  /**
   * Create a predicate given a date and (optionally) a format. If no format is specified, it is
    ...
    if (args.size() == 1) {
      ...
      try {
        ...
        throw new IllegalStateException(
            "Unable to process " + args.get(0) + " as a date using " + FORMAT.get().toPattern());
      }
    } else if (args.size() == 0) {
      ...
      try {
        ...
        throw new IllegalStateException(
            "Unable to process " + dateStr + " as a date using " + format);
      }
    }
  }
