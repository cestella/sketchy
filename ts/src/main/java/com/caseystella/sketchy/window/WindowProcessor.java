...
/**
 * The WindowProcessor instance provides the parser callbacks for the Window selector language. This constructs a Window
 * object to be used to compute sparse window intervals across time.
 */
public class WindowProcessor extends WindowBaseListener {
    ...
    /**
     * When we enter a specifier then we want to push onto the stack the specifier marker so we know when the specifier
     * parameters end.
     * 
     * @param ctx
     */
    ...
    /**
     * Read the specifier params off the stack in FIFO order until we get to the specifier marker. Now we can construct
     * the specifier, which is a Function which constructs a Selector Predicate based on the args passed to the selector
     * e.g. holidays:us:nyc would have 2 args us and nyc.
     *
     * @param ctx
     */
    @Override
    ...
    public void exitDay_specifier(WindowParser.Day_specifierContext ctx) {
        ...
        if (specifier.length() == 0 && ctx.exception != null) {
            IllegalStateException ise = new IllegalStateException("Invalid day specifier: " + ctx.getStart().getText(),
                    ctx.exception);
            throwable = ise;
            throw ise;
        }
    ...
    /**
     * When we're beginning an exclusion specifier list, then we push the list token so we know when we're done
     * processing
     * 
     * @param ctx
     */
    ...
    /**
     * And when we're done with the exclusions specifier, then we set the exclusions to the predicates we've put on the
     * stack.
     * 
     * @param ctx
     */
    ...
    /**
     * When we're beginning an inclusion specifier list, then we push the list token so we know when we're done
     * processing
     * 
     * @param ctx
     */
    ...
    /**
     * And when we're done with the inclusions specifier, then we set the exclusions to the predicates we've put on the
     * stack.
     * 
     * @param ctx
     */
    ...
    /**
     * If we have a total time interval that we've specified, then we want to set the interval. NOTE: the interval will
     * be set based on the smallest to largest being the start and end time respectively. Thus 'from 1 hour ago to 1 day
     * ago' and 'from 1 day ago to 1 hour ago' are equivalent.
     * 
     * @param ctx
     */
    @Override
     ...
     *
     * @param statement
     * @return Window returns a Window object (parameterized by time) from a statement specifying the window intervals
     *         conforming to the Window grammar.
    ...
    /**
     * Create a textual representation of the syntax tree. This is useful for those intrepid souls who wish to extend
     * the window selector language. God speed.
     * 
     * @param statement
