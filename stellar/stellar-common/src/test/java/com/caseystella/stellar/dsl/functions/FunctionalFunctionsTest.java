...
public class FunctionalFunctionsTest {
    ...
    public void testZipLongest_boundary() {
        for (String expr : ImmutableList.of("ZIP_LONGEST()", "ZIP_LONGEST( null, null )", "ZIP_LONGEST( [], null )",
                "ZIP_LONGEST( [], [] )", "ZIP_LONGEST( null, [] )")) {
    ...
    public void testZip_longest() {
        Map<String, Object> variables = ImmutableMap.of("list1", ImmutableList.of(1, 2, 3), "list2",
        ...
        for (String expr : ImmutableList.of("ZIP_LONGEST(list1, list2)", "ZIP_LONGEST( [1, 2, 3], [4, 5, 6, 7] )")) {
            List<List<Object>> o = (List<List<Object>>) run(expr, variables);
        ...
        for (String expr : ImmutableList.of(
                ...
                "REDUCE(ZIP_LONGEST(list1, list2), (s, x) -> s + GET_FIRST(x) * GET_LAST(x), 0)" // this works because
                                                                                                 // stellar treats nulls
                                                                                                 // as 0 in arithmetic
                                                                                                 // operations.
                ,
    ...
    public void testZip_boundary() {
        for (String expr : ImmutableList.of("ZIP()", "ZIP( null, null )", "ZIP( [], null )", "ZIP( [], [] )",
                "ZIP( null, [] )")) {
            List<List<Object>> o = (List<List<Object>>) run(expr, new HashMap<>());
            assertEquals(0, o.size());
        }
    ...
    public void testZip() {
        ...
        for (String expr : ImmutableList.of("REDUCE(ZIP(list1, list2), (s, x) -> s + GET_FIRST(x) * GET_LAST(x), 0)",
    ...
    public void testRecursive() {
        for (String expr : ImmutableList.of("MAP(list, inner_list -> REDUCE(inner_list, (x, y) -> x + y, 0) )",
            ...
            Object o = run(expr,
                    ImmutableMap.of("list", ImmutableList.of(ImmutableList.of(1, 2, 3), ImmutableList.of(4, 5, 6))));
    ...
    public void testMap_null() {
        for (String expr : ImmutableList.of("MAP([ 1, 2, null], x -> if x == null then 0 else 2*x )",
                "MAP([ 1, 2, null], x -> x == null ? 0 : 2*x )", "MAP([ 1, foo, baz], x -> x == null ? 0 : 2*x )")) {
    ...
    public void testMap() {
        for (String expr : ImmutableList.of("MAP([ 'foo', 'bar'], (x) -> TO_UPPER(x) )",
                ...
                "MAP([ foo, bar], x -> TO_UPPER(x) )", "MAP([ foo, bar], x -> true?TO_UPPER(x):THROW('error') )",
    ...
    public void testFilter() {
        for (String expr : ImmutableList.of("FILTER([ 'foo', 'bar'], (item) -> item == 'foo' )",
                "FILTER([ 'foo', bar], (item) -> item == 'foo' )", "FILTER([ foo, bar], (item) -> item == 'foo' )",
    ...
    public void testFilter_notnull() {
        for (String expr : ImmutableList.of("FILTER([ 'foo', null], item -> item != null )",
                "FILTER([ 'foo', baz], (item) -> item != null )", "FILTER([ foo, baz], (item) -> item != null )")) {
            Map<String, Object> variableMap = new HashMap<String, Object>() {
                {
                    put("foo", "foo");
                    put("bar", "bar");
