...
public class StellarArithmeticTest {
    ...
    public void testNumericOperations() {
        ...
        }
        {
            ...
            assertEquals(4,
                    (Integer) StellarProcessorUtils.run(query, ImmutableMap.of("one", 1, "very_nearly_one", 1.000001)),
                    1e-6);
        }
        {
    ...
    public void verifyExpectedReturnTypes() {
        ...
        expectedReturnTypeMappings.forEach((pair, expectedClass) -> {
            assertTrue(StellarProcessorUtils.run(pair.getLeft() + " * " + pair.getRight(), ImmutableMap.of())
