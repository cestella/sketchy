...
public class ShellFunctionsTest {
    ...
    Map<String, VariableResult> variables = ImmutableMap.of("var1",
            VariableResult.withExpression("CASEY", "TO_UPPER('casey')"), "var2", VariableResult.withValue("foo"),
            "var3", VariableResult.withValue(null), "var4", VariableResult.withExpression(null, "blah"));

    Context context = new Context.Builder().with(Context.Capabilities.SHELL_VARIABLES, () -> variables).build();

    /**
     * ╔══════════╤═══════╤════════════╗ ║ VARIABLE │ VALUE │ EXPRESSION ║ ╠══════════╪═══════╪════════════╣ ║ foo │ 2.0
     * │ 1 + 1 ║ ╚══════════╧═══════╧════════════╝
     **/
    @Multiline
    ...
    /**
     * ╔══════════╤═══════╤════════════╗ ║ VARIABLE │ VALUE │ EXPRESSION ║ ╠══════════╧═══════╧════════════╣ ║ (empty) ║
     * ╚═══════════════════════════════╝
     **/
    @Multiline
    ...
    public void testListVarsWithoutVars() {
        Context context = new Context.Builder().with(Context.Capabilities.SHELL_VARIABLES, () -> new HashMap<>())
                .build();
        Object out = StellarProcessorUtils.run("SHELL_LIST_VARS()", new HashMap<>(), context);
    ...
    /**
     * ╔════════╤═══════╗ ║ KEY │ VALUE ║ ╠════════╪═══════╣ ║ field1 │ val1 ║ ╟────────┼───────╢ ║ field2 │ val2 ║
     * ╚════════╧═══════╝
     **/
    @Multiline
    ...
    public void testVars2Map() {
        Object out = StellarProcessorUtils.run("SHELL_VARS2MAP('var1', 'var2')", new HashMap<>(), context);
    ...
    public void testGetExpression() {
        Object out = StellarProcessorUtils.run("SHELL_GET_EXPRESSION('var1')", new HashMap<>(), context);
    ...
    public void testEdit() {
        ...
        Object out = StellarProcessorUtils.run("TO_UPPER(SHELL_EDIT(foo))", ImmutableMap.of("foo", "foo"), context);
