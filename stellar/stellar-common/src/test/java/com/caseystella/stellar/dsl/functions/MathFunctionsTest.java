...
public class MathFunctionsTest {
  ...
  public static Object run(String rule, Map<String, Object> variables) {
    ...
    return processor.parse(rule,
        new DefaultVariableResolver(v -> variables.get(v), v -> variables.containsKey(v)),
  ...
  public void testIsNanWithNoArgs() {
    assertThrows(ParseException.class,
        () -> StellarProcessorUtils.runPredicate("IS_NAN()", new HashMap<>()));
  }
      ...
      for (String expr : ImmutableList.of(func + "(value)", func + "(" + test.getKey() + ")")) {
        if (Double.isNaN(test.getValue())) {
          ...
          assertEquals(test.getValue(),
              toDouble(run(expr, ImmutableMap.of("value", test.getKey()))), EPSILON,
