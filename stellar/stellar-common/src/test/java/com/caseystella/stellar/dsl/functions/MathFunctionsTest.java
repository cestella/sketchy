...
public class MathFunctionsTest {
    ...
    public static Object run(String rule, Map<String, Object> variables) {
        ...
        return processor.parse(rule, new DefaultVariableResolver(v -> variables.get(v), v -> variables.containsKey(v)),
                StellarFunctions.FUNCTION_RESOLVER(), context);
    }

    @Test
    public void testAbs() {
        assertValues("ABS", new HashMap<Double, Double>(baseExpectations) {
            {
                put(0d, 0d);
                put(10.5d, 10.5d);
                put(-10.5d, 10.5d);
    ...
    public void testSqrt() {
        assertValues("SQRT", new HashMap<Double, Double>(baseExpectations) {
            {
                put(0d, 0d);
                put(25d, 5d);
    ...
    public void testCeiling() {
        assertValues("CEILING", new HashMap<Double, Double>(baseExpectations) {
            {
                put(0d, 0d);
                put(10.5d, 11d);
                put(-10.5d, -10d);
    ...
    public void testFloor() {
        assertValues("FLOOR", new HashMap<Double, Double>(baseExpectations) {
            {
                put(0d, 0d);
                put(10.5d, 10d);
                put(-10.5d, -11d);
    ...
    @Test
    public void testSin() {
        assertValues("SIN", new HashMap<Double, Double>(baseExpectations) {
            {
                put(0d, 0d);
                put(Math.PI / 6, 0.5);
    ...
    @Test
    public void testCos() {
        assertValues("COS", new HashMap<Double, Double>(baseExpectations) {
            {
                put(0d, 1d);
    ...
    @Test
    public void testTan() {
        assertValues("TAN", new HashMap<Double, Double>(baseExpectations) {
            {
                put(0d, 0d);
    ...
    @Test
    public void testExp() {
        assertValues("EXP", new HashMap<Double, Double>(baseExpectations) {
            {
                put(0d, 1d);
    ...
    public void testRound() {
        assertValues("ROUND", new HashMap<Double, Double>(baseExpectations) {
            {
                put(0d, 0d);
                put(0.5d, 1d);
                put(0.4d, 0d);
    ...
    public void testIsNaN() {
        Assertions.assertTrue(StellarProcessorUtils.runPredicate("IS_NAN(NaN)", new HashMap<>()));
        Assertions.assertFalse(StellarProcessorUtils.runPredicate("IS_NAN(1.0)", new HashMap<>()));
        Assertions.assertTrue(StellarProcessorUtils.runPredicate("IS_NAN(0.0/0.0)", new HashMap<>()));
    }

    @Test
    public void testIsNanWithNotNumberType() {
        assertThrows(ParseException.class,
                () -> StellarProcessorUtils.runPredicate("IS_NAN('casey')", new HashMap<>()));
    }

    @Test
    public void testIsNanWithNoArgs() {
        assertThrows(ParseException.class, () -> StellarProcessorUtils.runPredicate("IS_NAN()", new HashMap<>()));
    }

    public void assertValues(String func, Map<Double, Double> expected) {
        for (Map.Entry<Double, Double> test : expected.entrySet()) {
            for (String expr : ImmutableList.of(func + "(value)", func + "(" + test.getKey() + ")")) {
                if (Double.isNaN(test.getValue())) {
                    assertTrue(Double.isNaN(toDouble(run(expr, ImmutableMap.of("value", test.getKey())))),
                            expr + " != NaN, where value == " + test.getKey());
                } else {
                    assertEquals(test.getValue(), toDouble(run(expr, ImmutableMap.of("value", test.getKey()))), EPSILON,
