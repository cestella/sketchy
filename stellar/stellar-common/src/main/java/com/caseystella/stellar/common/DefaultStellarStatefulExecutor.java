...
public class DefaultStellarStatefulExecutor implements StellarStatefulExecutor, Serializable {
  ...
  /**
   * Provides additional context for initializing certain Stellar functions. For example, references
   * to Zookeeper or HBase.
   */
  ...
  /**
   * Execute an expression and assign the result to a variable. The variable is maintained in the
   * context of this executor and is available to all subsequent expressions.
   *
   ...
   *        the values available to the expression from an individual message. The state maps a
   *        variable name to a variable's value.
   */
  @Override
  ...
  /**
   * Execute a Stellar expression and return the result. The internal state of the executor is not
   * modified.
   *
   * @param expression The expression to execute.
   * @param state Additional state available to the expression. This most often represents the
   *        values available to the expression from an individual message. The state maps a variable
   *        name to a variable's value.
  ...
  public <T> T execute(String expression, Map<String, Object> state, Class<T> clazz) {
    ...
    if (result == null) {
      throw new IllegalArgumentException(String.format(
          "Unexpected type: expected=%s, actual=%s, expression=%s", clazz.getSimpleName(),
