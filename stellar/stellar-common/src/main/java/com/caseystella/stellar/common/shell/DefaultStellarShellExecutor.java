...
import com.caseystella.stellar.common.shell.specials.Comment;
...
public class DefaultStellarShellExecutor implements StellarShellExecutor {
    ...
    public DefaultStellarShellExecutor(Properties properties, Optional<String> zookeeperUrl) throws Exception {
    ...
    public static List<SpecialCommand> defaultSpecials() {
        return Arrays.asList(new AssignmentCommand(), new DocCommand(), new QuitCommand(), new Comment(),
                new MagicListFunctions(), new MagicListVariables(), new MagicDefineGlobal(), new MagicUndefineGlobal(),
    ...
    /**
     * Retrieves the GLOBAL_CONFIG, if it exists. If it does not, it creates the GLOBAL_CONFIG and adds it to the
     * Stellar execution context.
     *
    ...
    /**
     * Returns all variables that have been defined. Unlike 'getState' this unwraps the VariableResult so that we have
     * the actual value.
     *
     ...
     * 
     * @param properties
     *            Properties to configure the context.
     * @param zkClient
     *            An optional Zookeeper client.
     */
    private Context createContext(Properties properties, Optional<CuratorFramework> zkClient) throws Exception {
        ...
        if (zkClient.isPresent()) {
            ...
            contextBuilder.with(Context.Capabilities.ZOOKEEPER_CLIENT, () -> zkClient.get());
        ...
        return contextBuilder.with(SHELL_VARIABLES, () -> variables)
                ...
                .with(Context.Capabilities.STELLAR_CONFIG, () -> getStellarConfig(globals, properties)).build();
    }
