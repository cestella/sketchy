...
/**
 * A MagicCommand that lists the variables available within the Stellar execution environment.
 *
 * %vars
 */
public class MagicListVariables implements SpecialCommand {
    ...
    public StellarResult execute(String command, StellarShellExecutor executor) {
        ...
        String vars = executor.getState().entrySet().stream().map(e -> format(e)).collect(Collectors.joining(", "));
