...
public class VFSClassloaderUtil {
  ...
  /**
   * Create a FileSystem manager suitable for our purposes. This manager supports files of the
   * following types: * res - resource files * jar * tar * bz2 * tgz * zip * HDFS * FTP * HTTP/S *
   * file
   * 
   * @return vfs
   ...
   * res - resource files * jar * tar * bz2 * tgz * zip * HDFS * FTP * HTTP/S * file
   * 
   * @param paths A set of comma separated paths. The paths are URIs or URIs with a regex pattern at
   *        the end.
  ...
  public static Optional<ClassLoader> configureClassloader(String paths)
    ...
    if (objects == null || objects.length == 0) {
      LOG.debug(
          "No Classloader able to be resolved from provided paths. Not returning a ClassLoader.");
      return Optional.empty();
    }
  ...
  /**
   * Resolve a set of URIs into FileObject objects. This is not recursive. The URIs can refer
   * directly to a file or directory or an optional regex at the end. (NOTE: This is NOT a glob).
   * 
