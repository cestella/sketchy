...
public enum ComparisonExpressionWithOperatorEvaluator {
  /**
   * The instance of {@link ComparisonExpressionWithOperatorEvaluator} used in order to evaluate
   * Stellar comparison expressions.
   */
  INSTANCE;
   ...
   * {@link EqualityOperatorsEvaluator#evaluate(Token, Token, StellarParser.ComparisonOpContext)}.
   * 3. Testing less than, less than or equal, greater than, and greater than or equal
   * {@link ComparisonOperatorsEvaluator} 4. Otherwise thrown {@link ParseException}.
   *
   ...
   * @return A token with type boolean. This is based on the comparison of the {@code right} and
   *         {@code left} values.
   */
  public Token<Boolean> evaluate(final Token<?> left, final Token<?> right,
    ...
    if (op.EQ() != null) {
      return new Token<>(Strategy.EQUALITY_OPERATORS.evaluator().evaluate(left, right, op),
          Boolean.class, context);
    } else if (op.NEQ() != null) {
      return new Token<>(!Strategy.EQUALITY_OPERATORS.evaluator().evaluate(left, right, op),
          Boolean.class, context);
    } else if (op.LT() != null || op.GT() != null || op.LTE() != null || op.GTE() != null) {
      return new Token<>(Strategy.COMPARISON_OPERATORS.evaluator().evaluate(left, right, op),
          Boolean.class, context);
    }

    throw new ParseException("Unsupported operations. The following expression is invalid: "
        + left.getValue() + op.getText() + right.getValue());
  }
}
