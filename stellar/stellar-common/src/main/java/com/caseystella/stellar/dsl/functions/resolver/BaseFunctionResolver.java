...
/**
 * The base implementation of a function resolver that provides a means for lazy initialization, thread-safety, and a
 * mechanism for function resolution.
 *
 * Concrete function resolvers can override the `resolvables` method which defines the classes that are interrogated
 * further to discover Stellar functions.
 */
public abstract class BaseFunctionResolver implements FunctionResolver, Serializable {
    ...
    public BaseFunctionResolver() {
        ...
        functions = Suppliers
                .memoize((Supplier<Map<String, StellarFunctionInfo>> & Serializable) this::resolveFunctions);
        closed = false;
    }
     ...
     * 
     * @throws IOException
     *             Catches all exceptions and summarizes them.
     */
    @Override
        ...
        if (!closed) {
            ...
            if (!errors.isEmpty()) {
                ...
                for (Map.Entry<String, Throwable> e : errors.entrySet()) {
                    ...
                    String eText = String.format("Exception - Function: %s; Message: %s; Cause: %s", e.getKey(),
        ...
        for (Class<? extends StellarFunction> clazz : resolvables()) {
            ...
            if (fn != null) {
                ...
                if (fnSameName != null && ObjectUtils.notEqual(fnSameName, fn)) {
                    LOG.warn("Namespace conflict: duplicate function names; `{}` implemented by [{}, {}]", fn.getName(),
                            fnSameName.getFunction(), fn.getFunction());
                }
     ...
     * 
     * @param clazz
     *            The class containing a Stellar function definition.
     */
