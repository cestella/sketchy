...
/**
 * The base implementation of a function resolver that provides a means for lazy initialization,
 * thread-safety, and a mechanism for function resolution.
 *
 * Concrete function resolvers can override the `resolvables` method which defines the classes that
 * are interrogated further to discover Stellar functions.
 */
public abstract class BaseFunctionResolver implements FunctionResolver, Serializable {
  ...
  public BaseFunctionResolver() {
    // memoize provides lazy initialization and thread-safety (the ugly cast is necessary for
    // serialization)
    functions = Suppliers.memoize(
        (Supplier<Map<String, StellarFunctionInfo>> & Serializable) this::resolveFunctions);
    closed = false;
  }
    ...
    if (!closed) {
      ...
      if (!errors.isEmpty()) {
        ...
        for (Map.Entry<String, Throwable> e : errors.entrySet()) {
          ...
          String eText = String.format("Exception - Function: %s; Message: %s; Cause: %s",
              e.getKey(), throwable.getMessage(), throwable.getCause());
  ...
  public static StellarFunctionInfo resolveFunction(Class<? extends StellarFunction> clazz) {
    ...
    if (clazz.isAnnotationPresent(Stellar.class)) {
      ...
      if (fullyQualifiedName != null && function != null) {
        info = new StellarFunctionInfo(annotation.description(), fullyQualifiedName,
            annotation.params(), annotation.returns(), function);
      }
    }
