...
public class StellarAssignment implements Map.Entry<String, Object> {
    ...
    public static boolean isAssignment(String statement) {
        return statement != null && statement.contains(":=") && // has the assignment operator
     ...
     *             implementations may, but are not required to, throw this exception if the entry has been removed from
     *             the backing map.
     */
    @Override
    ...
    /**
     * Replaces the value corresponding to this entry with the specified value (optional operation). (Writes through to
     * the map.) The behavior of this call is undefined if the mapping has already been removed from the map (by the
