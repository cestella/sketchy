...
public class StellarShell extends AeshConsoleCallback implements Completion {
  ...
  private List<TerminalCharacter> EXPRESSION_PROMPT = new ArrayList<TerminalCharacter>() {
    {
      ...
      add(new TerminalCharacter('S', new TerminalColor(Color.GREEN, Color.DEFAULT),
          CharacterType.BOLD));
      add(new TerminalCharacter('t', new TerminalColor(Color.GREEN, Color.DEFAULT),
          CharacterType.BOLD));
      add(new TerminalCharacter('e', new TerminalColor(Color.GREEN, Color.DEFAULT),
          CharacterType.BOLD));
      add(new TerminalCharacter('l', new TerminalColor(Color.GREEN, Color.DEFAULT),
          CharacterType.BOLD));
      add(new TerminalCharacter('l', new TerminalColor(Color.GREEN, Color.DEFAULT),
          CharacterType.BOLD));
      add(new TerminalCharacter('a', new TerminalColor(Color.GREEN, Color.DEFAULT),
          CharacterType.BOLD));
      add(new TerminalCharacter('r', new TerminalColor(Color.GREEN, Color.DEFAULT),
          CharacterType.BOLD));
      add(new TerminalCharacter(']', new TerminalColor(Color.RED, Color.DEFAULT)));
      add(new TerminalCharacter('>', new TerminalColor(Color.GREEN, Color.DEFAULT),
   ...
   * @throws IOException
   */
  private static void loadVariables(CommandLine commandLine, StellarShellExecutor executor)
      throws IOException {
    ...
    if (commandLine.hasOption("v")) {
      ...
      Map<String, Object> variables =
          JSONUtils.INSTANCE.load(new File(variablePath), JSONUtils.MAP_SUPPLIER);
   ...
   */
  private StellarShellExecutor createExecutor(CommandLine commandLine, Console console,
      Properties properties, StellarAutoCompleter autoCompleter) throws Exception {
    ...
    executor.getContext().addCapability(Context.Capabilities.CONSOLE, () -> console);

    // allows some Stellar functions to access Stellar internals; should probably use %magics
    // instead
    executor.getContext().addCapability(Context.Capabilities.SHELL_VARIABLES,
        () -> executor.getState());

    // register the auto-completer to be notified when needed
    executor
        .addSpecialListener((special) -> autoCompleter.addCandidateFunction(special.getCommand()));
    executor
        .addFunctionListener((function) -> autoCompleter.addCandidateFunction(function.getName()));
  ...
  /**
   * Retrieves the Stellar properties. The properties are either loaded from a file in the classpath
   * or a set of defaults are used.
   */
  ...
  public void run() {
    ...
    executor.getContext().getCapability(Context.Capabilities.GLOBAL_CONFIG, false)
