...
public class StellarShell extends AeshConsoleCallback implements Completion {
    ...
    private static void loadVariables(CommandLine commandLine, StellarShellExecutor executor) throws IOException {
        ...
        if (commandLine.hasOption("v")) {
            ...
            Map<String, Object> variables = JSONUtils.INSTANCE.load(new File(variablePath), JSONUtils.MAP_SUPPLIER);
     ...
     *            The command line arguments.
     * @param console
     *            The console which drives the REPL.
     * @param properties
     *            Stellar properties.
     */
    private StellarShellExecutor createExecutor(CommandLine commandLine, Console console, Properties properties,
        ...
        executor.getContext().addCapability(Context.Capabilities.SHELL_VARIABLES, () -> executor.getState());
    ...
    /**
     * Retrieves the Stellar properties. The properties are either loaded from a file in the classpath or a set of
     * defaults are used.
     */
