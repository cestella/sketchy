...
/**
 * This is an enum implementing a hashing strategy pattern. Because hash types may be quite
 * different, but have a similar interface we have laid on top this abstraction to allow new types
 * of hashes to be added easily.
 *
 * In order to add a new family of hashes, simply implement the Hasher interface and register it
 * with this enum. The search order for algorithms to their respective Hasher is in the order of the
...
public enum HashStrategy {
   ...
   * 
   * @param algorithm The algorithm to find a hasher handler for. Note: this is upper-cased prior to
   *        search
   * @param config The config for the hasher
   * @return The hasher which will handle the algorithm. If the algorithm is not supported by any
   *         registered hashers, an IllegalArgumentException is thrown.
   */
