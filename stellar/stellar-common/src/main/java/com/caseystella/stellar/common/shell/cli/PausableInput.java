...
/**
 * An input stream which mirrors System.in, but allows you to 'pause' and 'unpause' it. The Aeshell
 * has an external thread which is constantly polling System.in. If you need to spawn a program
 * externally (i.e. an editor) which shares stdin, this thread and the spawned program both share a
 * buffer. This causes contention and unpredictable results (e.g. an input may be consumed by either
 * the aeshell thread or the spawned program)
 *
 * Because you can inject an input stream into the console, we create this which can act as a facade
 * to System.in under normal 'unpaused' circumstances, and when paused, turn off the access to
 * System.in. This allows us to turn off access to aeshell while maintaining access to the external
 * program.
 *
 */
public class PausableInput extends InputStream {
   ...
   * stream is at the end of the file, the value <code>-1</code> is returned; otherwise, at least
   ...
   * in elements <code>b[0]</code> through <code>b[</code><i>k</i><code>-1]</code>, leaving elements
   ...
   * the <code>read(b,</code> <code>off,</code> <code>len)</code> method. If any subsequent call to
   ...
   * reaching end of file before <code>n</code> bytes have been skipped is only one possibility. The
   * actual number of bytes skipped is returned. If {@code n} is negative, the {@code skip} method
   * for class {@code InputStream} always returns 0, and no bytes are skipped. Subclasses may handle
   * the negative value differently.
   * <p>
   * <p>
   * The <code>skip</code> method of this class creates a byte array and then repeatedly reads into
   * it until <code>n</code> bytes have been read or the end of the stream has been reached.
   ...
   * the <code>read</code> method, followed by any bytes that otherwise would have been the next
   * input data as of the time of the call to <code>reset</code>.
   * </ul>
   * <p>
   ...
   * that depends on the particular type of the input stream and how it was created. The bytes that
   * will be supplied to subsequent callers of the <code>read</code> method depend on the particular
   ...
   * of a particular input stream instance. The <code>markSupported</code> method of
