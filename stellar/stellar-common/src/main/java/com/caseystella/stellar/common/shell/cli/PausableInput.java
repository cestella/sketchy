...
/**
 * An input stream which mirrors System.in, but allows you to 'pause' and 'unpause' it. The Aeshell has an external
 * thread which is constantly polling System.in. If you need to spawn a program externally (i.e. an editor) which shares
 * stdin, this thread and the spawned program both share a buffer. This causes contention and unpredictable results
 * (e.g. an input may be consumed by either the aeshell thread or the spawned program)
 *
 * Because you can inject an input stream into the console, we create this which can act as a facade to System.in under
 * normal 'unpaused' circumstances, and when paused, turn off the access to System.in. This allows us to turn off access
 * to aeshell while maintaining access to the external program.
 *
 */
public class PausableInput extends InputStream {
     ...
     * <p>
     * <p>
     * If the length of <code>b</code> is zero, then no bytes are read and <code>0</code> is returned; otherwise, there
     * is an attempt to read at least one byte. If no byte is available because the stream is at the end of the file,
     * the value <code>-1</code> is returned; otherwise, at least one byte is read and stored into <code>b</code>.
     * <p>
     * <p>
     ...
     * actually read; these bytes will be stored in elements <code>b[0]</code> through
     * <code>b[</code><i>k</i><code>-1]</code>, leaving elements <code>b[</code><i>k</i><code>]</code> through
     ...
     * only one possibility. The actual number of bytes skipped is returned. If {@code n} is negative, the {@code skip}
     * method for class {@code InputStream} always returns 0, and no bytes are skipped. Subclasses may handle the
     ...
     * instance. The <code>markSupported</code> method of <code>InputStream</code> returns <code>false</code>.
     *
