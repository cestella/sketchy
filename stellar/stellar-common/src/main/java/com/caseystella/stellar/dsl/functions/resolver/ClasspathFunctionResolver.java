...
import static com.caseystella.stellar.dsl.functions.resolver.ClasspathFunctionResolver.Config.STELLAR_SEARCH_EXCLUDES_KEY;
 ...
 *
 * By default, the entire classpath will be searched for Stellar functions. At times, this can take quite a while. To
 * shorten the search time, a property can be defined to either include or exclude certain packages. The fewer packages
 * there are to search, the quicker the search will be.
 *
 * The properties are pulled from the Context's 'STELLAR_CONFIG'. In the REPL, this is defined in a file called
 * 'stellar.properties' on the classpath.
 *
 ...
 *
 * The following property definition will exclude Stellar functions that are part of Metron's management suite of
 * function.
 *
 * stellar.function.resolver.excludes = org.apache.metron.management.*
 *
 * The following property definition would also exclude the Stellar functions that are part of the management suite of
 * functions. Of course, this may also exclude other packages, but this serves as an example of the types of expression
 * that can be used.
 *
...
public class ClasspathFunctionResolver extends BaseFunctionResolver {
    public enum Config {
        /**
         * The set of paths. These paths are comma separated URLs with optional regex patterns at the end. e.g.
         * hdfs://node1:8020/apps/metron/stellar/.*.jar,hdfs://node1:8020/apps/metron/my_org/.*.jar would signify all
         * the jars under /apps/metron/stellar and /apps/metron/my_org in HDFS.
         */
        STELLAR_VFS_PATHS("stellar.function.paths", ""),
        /**
         * The key for a global property that defines one or more regular expressions that specify what should be
         * included when searching for Stellar functions.
         */
        STELLAR_SEARCH_INCLUDES_KEY("stellar.function.resolver.includes", ""),
        /**
         * The key for a global property that defines one or more regular expressions that specify what should be
         * excluded when searching for Stellar functions.
         */
    ...
    /**
     * The includes and excludes can include a list of multiple includes or excludes that are delimited by these values.
     */
    private static final String STELLAR_SEARCH_DELIMS = "[,:]";

    /**
     * Regular expressions defining packages that should be included in the Stellar function resolution process.
     */
    private List<String> includes;

    /**
     * Regular expressions defining packages that should be excluded from the Stellar function resolution process.
     */
    ...
    /**
     * Includes one or more packages in the Stellar function resolution process. The packages to include can be
    ...
    /**
     * Excludes one or more packages from the Stellar function resolution process. The packages to exclude can be
     * specified with a regular expression.
     * 
     * @param toExclude
     *            The regular expressions defining packages that should be excluded.
     */
    ...
    /**
     * Returns a set of classes that should undergo further interrogation for resolution (aka discovery) of Stellar
     * functions.
     */
    @Override
    @SuppressWarnings("unchecked")
