...
import com.caseystella.stellar.common.LambdaExpression;
...
public class FunctionalFunctions {
    ...
    @Stellar(name = "MAP", description = "Applies lambda expression to a list of arguments. e.g. MAP( [ 'foo', 'bar' ] , ( x ) -> TO_UPPER(x) ) would yield [ 'FOO', 'BAR' ]", params = {
            "list - List of arguments.",
            "transform_expression - The lambda expression to apply. This expression is assumed to take one argument." }, returns = "The input list transformed item-wise by the lambda expression.")
    ...
    @Stellar(name = "FILTER", description = "Applies a filter in the form of a lambda expression to a list. e.g. FILTER( [ 'foo', 'bar' ] , (x) -> x == 'foo') would yield [ 'foo']", params = {
            "list - List of arguments.",
            "predicate - The lambda expression to apply.  This expression is assumed to take one argument and return a boolean." }, returns = "The input list filtered by the predicate.")
    ...
    @Stellar(name = "REDUCE", description = "Reduces a list by a binary lambda expression. That is, the expression takes two arguments.  Usage example: REDUCE( [ 1, 2, 3 ] , (x, y) -> x + y) would sum the input list, yielding 6.", params = {
            "list - List of arguments.",
            ...
            "initial_value - The initial value to use." }, returns = "The reduction of the list.")
    ...
    @Stellar(name = "ZIP_LONGEST", description = "Zips lists into a single list where the ith element is an list "
            ...
            + "and [wikipedia](https://en.wikipedia.org/wiki/Convolution_(computer_science)) for more context.", params = {
                    "list(s) - List(s) to zip." }, returns = "The zip of the lists.  The returned list is the max size of all the lists.  "
    ...
    @Stellar(name = "ZIP", description = "Zips lists into a single list where the ith element is an list containing the ith items from the constituent lists. "
            + "See [python](https://docs.python.org/3/library/functions.html#zip) and [wikipedia](https://en.wikipedia.org/wiki/Convolution_(computer_science)) for more context.", params = {
                    "list(s) - List(s) to zip." }, returns = "The zip of the lists.  The returned list is the min size of all the lists.  "
    ...
    private static List<List<Object>> zip(List<Object> args, boolean jagged) {
        ...
        return IntStream.range(0, resultSize).mapToObj(i -> {
