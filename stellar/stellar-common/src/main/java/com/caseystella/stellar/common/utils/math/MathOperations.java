...
public enum MathOperations implements Function<Number[], Number> {
    ABS(d -> Math.abs(d)), CEIL(d -> Math.ceil(d)), COS(d -> Math.cos(d)), FLOOR(d -> Math.floor(d)), LOG10(
            d -> Math.log10(d)), LOG2(d -> Math.log(d) / Math.log(2)), LN(d -> Math.log(d)), SIN(
                    d -> Math.sin(d)), SQRT(d -> Math.sqrt(d)), TAN(
