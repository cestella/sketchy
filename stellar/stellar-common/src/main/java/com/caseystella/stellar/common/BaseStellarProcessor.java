...
public class BaseStellarProcessor<T> {
  ...
  /**
   * The default expression cache. This is used when the expression cache is not otherwise
   * specified.
   */
  private static Cache<String, StellarCompiler.Expression> defaultExpressionCache;
  static {
    defaultExpressionCache =
        createCache(DEFAULT_CACHE_SIZE, DEFAULT_EXPIRY_TIME, DEFAULT_EXPIRY_TIME_UNITS);
  }
  /**
  ...
  /**
   * @param clazz The class containing the type that the Stellar expression being processed will
   *        evaluate to.
   */
  ...
  static Cache<String, StellarCompiler.Expression> createCache(int cacheSize, int expiryTime,
      TimeUnit expiryUnit) {
    CacheLoader<String, StellarCompiler.Expression> loader = key -> compile(key);
    return Caffeine.newBuilder().maximumSize(cacheSize).expireAfterAccess(expiryTime, expiryUnit)
        .build(loader);
  }

  /**
   * Parses the given rule and returns a set of variables that are used in the given Stellar
   * expression, {@code rule}.
   *
   ...
   *        the Stellar expression, {@code rule}.
  ...
  private ParseException createException(String rule, VariableResolver resolver, Throwable t) {
    ...
    return new ParseException(
        message + " with relevant variables " + Joiner.on(",").join(messagesUsed), t);
  }
   ...
   * 
   * @param rule The Stellar expression to parse and evaluate.
   * @return The Expression, which can be reevaluated without reparsing in different Contexts and
   *         Resolvers.
   */
   ...
   * will be returned otherwise a {@link ParseException} is thrown. If it is desired to return a
   * boolean whether the rule is valid or not, use the {@link #validate(String, boolean, Context)
   * validate} method. It is important to note that all variables will resolve to 'null.'
   *
   ...
   * @throws ParseException The exception containing the information as to why the expression is not
   *         valid.
   */
   ...
   * The rule is only meant to be validated. Validate in this instance means check whether or not
   * the rule is syntactically valid and whether the functions used exist. For example, it will not
   * check for variables that are not defined. Currently all variables resolve to 'null.' This is
   * mainly to make sure things function as expected when values are null.
   *
   * @param rule The Stellar transformation to validate.
   * @param throwException If true an invalid Stellar transformation will throw a
   *        {@link ParseException} otherwise a boolean will be returned.
   * @param context The Stellar context to be used when validating the Stellar transformation.
   * @return If {@code throwException} is true and {@code rule} is invalid a {@link ParseException}
   *         is thrown. If {@code throwException} is false and {@code rule} is invalid then false is
   *         returned. Otherwise true if {@code rule} is valid, false if {@code rule} is invalid.
  ...
  public boolean validate(final String rule, final boolean throwException, final Context context)
    ...
    try {
      parse(rule, DefaultVariableResolver.NULL_RESOLVER(), StellarFunctions.FUNCTION_RESOLVER(),
          context);
    } catch (Throwable t) {
      if (throwException) {
