...
import com.caseystella.stellar.common.generated.StellarLexer;
 ...
 *
 * @param <T>
 *            The type that the processor expects to return after processing a Stellar expression.
...
public class BaseStellarProcessor<T> {
    ...
    static Cache<String, StellarCompiler.Expression> createCache(int cacheSize, int expiryTime, TimeUnit expiryUnit) {
        ...
        return Caffeine.newBuilder().maximumSize(cacheSize).expireAfterAccess(expiryTime, expiryUnit).build(loader);
    }
     ...
     *
     * @param rule
     *            The Stellar expression to find out what variables are used.
     ...
     * 
     * @param rule
     *            The Stellar expression to parse and evaluate.
     * @param variableResolver
     *            The {@link VariableResolver} to determine values of variables used in the Stellar expression,
     *            {@code rule}.
     * @param functionResolver
     *            The {@link FunctionResolver} to determine values of functions used in the Stellar expression,
     *            {@code rule}.
     * @param context
     *            The context used during validation.
     * @return The value of the evaluated Stellar expression, {@code rule}.
     */
    public T parse(final String rule, final VariableResolver variableResolver, final FunctionResolver functionResolver,
        ...
        if (context.getActivityType() == null) {
            context.setActivityType(Context.ActivityType.PARSE_ACTIVITY);
        }
        try {
            expression = expressionCache.get(rule, r -> compile(r));
        } catch (Throwable e) {
    ...
    public static StellarCompiler.Expression compile(final String rule) {
        ...
        StellarCompiler treeBuilder = new StellarCompiler(ArithmeticEvaluator.INSTANCE, NumberLiteralEvaluator.INSTANCE,
    ...
    /**
     * This method determines if a given rule is valid or not. If the given rule is valid then true will be returned
     * otherwise a {@link ParseException} is thrown. If it is desired to return a boolean whether the rule is valid or
     * not, use the {@link #validate(String, boolean, Context) validate} method. It is important to note that all
     * variables will resolve to 'null.'
     *
     * @param rule
     *            The rule to validate.
     * @return If the given rule is valid then true otherwise an exception is thrown.
     * @throws ParseException
     *             If the rule is invalid an exception of this type is thrown.
     */
     ...
     * 
     * @param rule
     *            The Stellar expression to validate.
     * @param context
     *            The context used to validate the Stellar expression.
     * @return If valid Stellar expression true, otherwise an exception will be thrown.
     * @throws ParseException
     *             The exception containing the information as to why the expression is not valid.
     */
    ...
    /**
     * Here it is not desirable to add our custom listener. It is not the intent to evaluate the rule. The rule is only
     * meant to be validated. Validate in this instance means check whether or not the rule is syntactically valid and
     * whether the functions used exist. For example, it will not check for variables that are not defined. Currently
     * all variables resolve to 'null.' This is mainly to make sure things function as expected when values are null.
     *
     * @param rule
     *            The Stellar transformation to validate.
     * @param throwException
     *            If true an invalid Stellar transformation will throw a {@link ParseException} otherwise a boolean will
     *            be returned.
     * @param context
     *            The Stellar context to be used when validating the Stellar transformation.
     * @return If {@code throwException} is true and {@code rule} is invalid a {@link ParseException} is thrown. If
     *         {@code throwException} is false and {@code rule} is invalid then false is returned. Otherwise true if
     *         {@code rule} is valid, false if {@code rule} is invalid.
     * @throws ParseException
     *             Thrown if {@code rule} is invalid and {@code throwException} is true.
     */
    public boolean validate(final String rule, final boolean throwException, final Context context)
        ...
        context.setActivityType(Context.ActivityType.VALIDATION_ACTIVITY);
        try {
