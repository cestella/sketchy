...
public class RegExFunctions {
    ...
    @Stellar(name = "REGEXP_MATCH", description = "Determines whether a regex matches a string, if a list of patterns is passed, then the matching is an OR operation", params = {
            "string - The string to test",
            "pattern - The proposed regex pattern or a list of proposed regex patterns" }, returns = "True if the regex pattern matches the string and false if otherwise.")
    ...
    @Stellar(name = "REGEXP_GROUP_VAL", description = "Returns the value of a group in a regex against a string", params = {
            "string - The string to test", "pattern - The proposed regex pattern",
            "group - integer that selects what group to select, starting at 1" }, returns = "The value of the group, or null if not matched or no group at index")
    public static class RegexpGroupValue extends BaseStellarFunction {

        @Override
        public Object apply(List<Object> list) {
            if (list.size() != 3) {
                throw new IllegalStateException("REGEXP_GROUP_VAL expects three args: [string, pattern, int]" + ""
    ...
    @Stellar(name = "REGEXP_REPLACE", description = "Replace all occurences of the regex pattern within the string by value", params = {
            "string - The input string",
            "pattern - The regex pattern to be replaced. Special characters must be escaped (e.g. \\\\d)",
            "value - The value to replace the regex pattern" }, returns = "The modified input string with replaced values")
    public static class RegexpReplace extends BaseStellarFunction {

        @Override
        public Object apply(List<Object> list) {
            if (list.size() != 3) {
                throw new IllegalStateException("REGEXP_REPLACE expects three args: [string, pattern, value]"
                        + " where pattern is a regexp pattern");
            }
            String str = (String) list.get(0);
            String stringPattern = (String) list.get(1);
