...
public class NetworkFunctions {
  @Stellar(name = "IN_SUBNET", description = "Returns true if an IP is within a subnet range.",
      params = {"ip - The IP address in string form",
    ...
    public Object apply(List<Object> list) {
      if (list.size() < 2) {
        throw new IllegalStateException(
            "IN_SUBNET expects at least two args: [ip, cidr1, cidr2, ...]"
  ...
  @Stellar(name = "REMOVE_SUBDOMAINS", namespace = "DOMAIN",
      description = "Removes the subdomains from a domain.",
      params = {"domain - Fully qualified domain name"},
    ...
    public Object apply(List<Object> objects) {
      ...
      if (idn != null) {
        ...
        if (!StringUtils.isEmpty(dn)) {
          ...
          String hostnameWithoutSubsAndTLD =
              Iterables.getLast(Splitter.on(".").split(hostnameWithoutTLD), null);
  ...
  @Stellar(name = "REMOVE_TLD", namespace = "DOMAIN",
      description = "Removes the top level domain (TLD) suffix from a domain.",
      params = {"domain - Fully qualified domain name"}, returns = "The domain without the TLD.  "
  ...
  @Stellar(name = "TO_TLD", namespace = "DOMAIN",
      description = "Extracts the top level domain from a domain",
      params = {"domain - Fully qualified domain name"}, returns = "The TLD of the domain.  "
  ...
  @Stellar(name = "TO_PATH", namespace = "URL", description = "Extract the path from a URL.",
      params = {"url - URL in String form"},
  ...
  @Stellar(name = "TO_HOST", namespace = "URL", description = "Extract the hostname from a URL.",
      params = {"url - URL in String form"},
  ...
  @Stellar(name = "TO_PROTOCOL", namespace = "URL",
      description = "Extract the protocol from a URL.", params = {"url - URL in String form"},
  ...
  /**
   * Extract the TLD. If the domain is a normal domain, then we can handle the TLD via the
   * InternetDomainName object. If it is not, then we default to returning the last segment after
   * the final '.'
   * 
   * @param idn
   * @param dn
    ...
    if (dnObj != null) {
      if (dnObj instanceof String) {
        ...
        throw new IllegalArgumentException(
            dnObj + " is not a string and therefore also not a domain.");
      }
    }
    return null;
  }
