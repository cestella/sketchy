...
import com.caseystella.stellar.common.encoding.Encodings;
...
public class EncodingFunctions {
    ...
    @Stellar(name = "GET_SUPPORTED_ENCODINGS", description = "Returns a list of the encodings that are currently supported as a list", params = {}, returns = "A list of supported encodings")
    ...
    @Stellar(name = "IS_ENCODING", description = "Returns if the passed string is encoded in one of the supported encodings", params = {
            "string - the string to test", "encoding - the encoding to test, must be one of encodings returned from "
                    + "LIST_SUPPORTED_ENCODINGS" }, returns = "true if it is encoded, false if not")
    ...
    @Stellar(name = "DECODE", description = "Decodes the passed string with the provided encoding, "
            + " must be one of the encodings returned from LIST_SUPPORTED_ENCODINGS", params = {
                    ...
                    "verify - (optional), true or false to determine if string should be verified as being "
                            + "encoded with the passed encoding" }, returns = "The decoded string on success\n"
    ...
    @Stellar(name = "ENCODE", description = "Encodes the passed string with the provided encoding, "
            + " must be one of the encodings returned from LIST_SUPPORTED_ENCODINGS", params = {
                    "string - the string to encode",
                    "encoding - the encoding to use, must be one of encodings returned from "
                            + "LIST_SUPPORTED_ENCODINGS" }, returns = "The encoded string or null on error")
