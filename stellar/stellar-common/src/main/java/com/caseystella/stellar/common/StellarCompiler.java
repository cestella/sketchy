...
public class StellarCompiler extends StellarBaseListener {
  ...
  public static class Expression implements Serializable {
    ...
    private boolean isConditionalContext(Class<?> tokenValueType) {
      return tokenValueType != null && (tokenValueType == BooleanArg.class
          || tokenValueType == IfExpr.class || tokenValueType == MatchClauseCheckExpr.class);
    }
    ...
    public Object apply(ExpressionState state) {
      Deque<Token<?>> instanceDeque = new ArrayDeque<>();
      {
        ...
        for (Iterator<Token<?>> it = getTokenDeque().descendingIterator(); it.hasNext();) {
          token = it.next();
          // if we've skipped an else previously, then we need to skip the deferred tokens
          ...
          /*
           * curr is the current value on the stack. This is the non-deferred actual evaluation for
           * this expression and with the current context.
           */
          Token<?> curr = instanceDeque.peek();
          boolean isFalsey = curr != null
              && (isBoolean(token, curr.getValue()) || isEmptyList(token, curr.getValue()));
          if (isFalsey) {
            // If we're in a situation where the token is a boolean token and the current value is
            // one of the implicitly falsey scenarios
          ...
          if (curr != null && curr.getValue() != null && curr.getValue() instanceof Boolean
            ...
            // if we have a boolean as the current value and the next non-contextual token is a
            // short circuit op
      ...
      if (instanceDeque.isEmpty()) {
        throw new ParseException(
            "Invalid parse, stack not empty: " + Joiner.on(',').join(instanceDeque));
      } else {
  ...
  public void exitArithExpr_div(StellarParser.ArithExpr_divContext ctx) {
    ...
    expression.tokenDeque.push(new Token<>((tokenDeque, state) -> {
      Pair<Token<? extends Number>, Token<? extends Number>> p = getArithExpressionPair(tokenDeque);
      tokenDeque.push(arithmeticEvaluator
          .evaluate(ArithmeticEvaluator.ArithmeticEvaluatorFunctions.division(context), p));
    }, DeferredFunction.class, context));
  }
  ...
  public void exitArithExpr_mul(StellarParser.ArithExpr_mulContext ctx) {
    ...
    expression.tokenDeque.push(new Token<>((tokenDeque, state) -> {
      Pair<Token<? extends Number>, Token<? extends Number>> p = getArithExpressionPair(tokenDeque);
      tokenDeque.push(arithmeticEvaluator
          .evaluate(ArithmeticEvaluator.ArithmeticEvaluatorFunctions.multiplication(context), p));
    }, DeferredFunction.class, context));
  }
  ...
  public void exitElse_expr(StellarParser.Else_exprContext ctx) {
    expression.tokenDeque
        .push(new Token<>(new EndConditional(), EndConditional.class, getArgContext()));
  }
  ...
  public void exitNotFunc(StellarParser.NotFuncContext ctx) {
    ...
    expression.tokenDeque.push(new Token<>((tokenDeque, state) -> {
      ...
      Boolean v =
          Optional.ofNullable(ConversionUtils.convert(arg.getValue(), Boolean.class)).orElse(false);
  ...
  public void exitStringLiteral(StellarParser.StringLiteralContext ctx) {
    ...
    expression.tokenDeque.push(
        new Token<>(literal.substring(1, literal.length() - 1), String.class, getArgContext()));
  }
  ...
  public void exitLogicalExpressionAnd(StellarParser.LogicalExpressionAndContext ctx) {
    ...
    expression.tokenDeque
        .push(new Token<>(new ShortCircuitFrame(), ShortCircuitFrame.class, context));
  }
  ...
  private boolean booleanOp(final Token<?> left, final Token<?> right, final BooleanOp op,
      final String opName) {
    Boolean l =
        Optional.ofNullable(ConversionUtils.convert(left.getValue(), Boolean.class)).orElse(false);
    Boolean r =
        Optional.ofNullable(ConversionUtils.convert(right.getValue(), Boolean.class)).orElse(false);
    return op.op(l, r);
  }
  ...
  private void exitLambda(boolean hasArgs) {
    ...
    final List<String> variables =
        hasArgs ? (List<String>) instanceDeque.removeLast().getValue() : new ArrayList<>();
    expression.tokenDeque.push(new Token<>((tokenDeque, state) -> {
  ...
  private List<Object> getFunctionArguments(final Token<?> token) {
    if (token.getUnderlyingType().equals(List.class)) {
      ...
      throw new ParseException(
          "Unable to process in clause because " + token.getValue() + " is not a set");
    }
  }
  ...
  private StellarFunction resolveFunction(FunctionResolver functionResolver, String funcName) {
    try {
      ...
      String error =
          format("Unable to resolve function named '%s'.  Valid functions are %s", funcName, valid);
  ...
  public void enterFunc_args(StellarParser.Func_argsContext ctx) {
    expression.tokenDeque
        .push(new Token<>(new FunctionMarker(), FunctionMarker.class, getArgContext()));
  }
  ...
  public void exitMatchClauseCheckExpr(StellarParser.MatchClauseCheckExprContext ctx) {
    ...
    expression.tokenDeque
        .push(new Token<>(new MatchClauseCheckExpr(), MatchClauseCheckExpr.class, getArgContext()));
  }
  ...
  public void exitMatch_clause(StellarParser.Match_clauseContext ctx) {
    expression.tokenDeque
        .push(new Token<>(new MatchClauseEnd(), MatchClauseEnd.class, getArgContext()));
  }

  @Override
  public void exitMatchClauses(StellarParser.MatchClausesContext ctx) {
    expression.tokenDeque
        .push(new Token<>(new MatchClausesEnd(), MatchClausesEnd.class, getArgContext()));
  }
  ...
  public void exitComparisonExpressionWithOperator(
    ...
    expression.tokenDeque.push(new Token<>((tokenDeque, state) -> {
      ...
      tokenDeque.push(comparisonExpressionWithOperatorEvaluator.evaluate(left, right,
          (StellarParser.ComparisonOpContext) op, context));
    }, DeferredFunction.class, context));
  }
