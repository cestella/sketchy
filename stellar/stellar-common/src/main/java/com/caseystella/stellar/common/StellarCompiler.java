...
public class StellarCompiler extends StellarBaseListener {
    ...
    public static class ExpressionState {
        ...
        public ExpressionState(Context context, FunctionResolver functionResolver, VariableResolver variableResolver) {
    ...
    public static class Expression implements Serializable {
        ...
        private boolean isConditionalContext(Class<?> tokenValueType) {
            return tokenValueType != null && (tokenValueType == BooleanArg.class || tokenValueType == IfExpr.class
        ...
        public Object apply(ExpressionState state) {
            Deque<Token<?>> instanceDeque = new ArrayDeque<>();
            {
                ...
                for (Iterator<Token<?>> it = getTokenDeque().descendingIterator(); it.hasNext();) {
                    token = it.next();
                    // if we've skipped an else previously, then we need to skip the deferred tokens associated with the
                    // else.
                    ...
                    /*
                     * curr is the current value on the stack. This is the non-deferred actual evaluation for this
                     * expression and with the current context.
                     */
                    Token<?> curr = instanceDeque.peek();
                    boolean isFalsey = curr != null
                            && (isBoolean(token, curr.getValue()) || isEmptyList(token, curr.getValue()));
                    if (isFalsey) {
                        // If we're in a situation where the token is a boolean token and the current value is one of
                    ...
                    if (curr != null && curr.getValue() != null && curr.getValue() instanceof Boolean
                        ...
                        // if we have a boolean as the current value and the next non-contextual token is a short
                        // circuit op
    ...
    public void exitStringLiteral(StellarParser.StringLiteralContext ctx) {
        ...
        expression.tokenDeque
                .push(new Token<>(literal.substring(1, literal.length() - 1), String.class, getArgContext()));
    }
     ...
     * 
     * @param token
     *            The token containing the function arguments.
     * @return
     */
    @SuppressWarnings("unchecked")
