...
/**
 * {@link ComparisonOperatorsEvaluator} is used to evaluate comparison expressions using the
 * following operator {@literal '<', '<=', '>', or '>='.} There are four major cases when evaluating
 * a comparison expression.
 */
public class ComparisonOperatorsEvaluator implements ComparisonExpressionEvaluator {

  /**
   * 1. If either the left or right's value is null then return false. 2. If both sides of the
   * expression are instances of {@link Number} then: 1. If either side is a {@link Double} then get
   * {@link Number#doubleValue()} from both sides and compare using given operator. 2. Else if
   * either side is a {@link Float} then get {@link Number#floatValue()} from both sides and compare
   * using given operator. 3. Else if either side is a {@link Long} then get
   * {@link Number#longValue()} from both sides and compare using given operator. 4. Otherwise get
   * {@link Number#intValue()} from both sides and compare using given operator. 3. If both sides
   * are of the same type and implement the {@link Comparable} interface then use {@code compareTo}
   * method. 4. If none of the above are met then a {@link ParseException} is thrown.
   *
   ...
   * @param op This is a representation of a comparison operator
   *        {@literal (eg. <, <=, >, >=, ==, !=) }
   * @return A boolean value based on the comparison of {@code left} and {@code right}.
   */
  @Override
  public boolean evaluate(final Token<?> left, final Token<?> right,
    ...
    throw new ParseException("Unsupported operations. The following expression is invalid: "
        + left.getValue() + op + right.getValue());
  }

  /**
   * This method uses the inputs' ability to compare with one another's values by using the
   * {@code compareTo} method. It will use this and the operator to evaluate the output.
   *
   ...
   * @return A boolean value representing the comparison of the two values with the given operator.
   *         For example, {@code 1 <= 1} would be true.
   */
  @SuppressWarnings("unchecked")
  private <T extends Comparable> boolean compare(final T l, final T r,
      final StellarParser.ComparisonOpContext op) {
   ...
   *         For example, {@code 1 <= 1} would be true.
   */
  private boolean compareNumbers(final Number l, final Number r,
      final StellarParser.ComparisonOpContext op) {
    if (op.LT() != null) {
      return lessThan(l, r);
    } else if (op.LTE() != null) {
  ...
  /**
   * If the left side of the expression is less than or equal to the right then true otherwise
   * false.
   *
   * @param l The value of the left side of the expression.
   * @param r The value of the right side of the expression.
   * @return If the left side of the expression is less than or equal to the right then true
   *         otherwise false.
   */
  ...
  /**
   * If the left side of the expression is greater than or equal to the right then true otherwise
   * false.
   *
   * @param l The value of the left side of the expression.
   * @param r The value of the right side of the expression.
   * @return If the left side of the expression is greater than or equal to the right then true
   *         otherwise false.
   */
