 ...
 * {@literal '<', '<=', '>', or '>='.} There are four major cases when evaluating a comparison expression.
 */
public class ComparisonOperatorsEvaluator implements ComparisonExpressionEvaluator {

    /**
     * 1. If either the left or right's value is null then return false. 2. If both sides of the expression are
     * instances of {@link Number} then: 1. If either side is a {@link Double} then get {@link Number#doubleValue()}
     * from both sides and compare using given operator. 2. Else if either side is a {@link Float} then get
     * {@link Number#floatValue()} from both sides and compare using given operator. 3. Else if either side is a
     * {@link Long} then get {@link Number#longValue()} from both sides and compare using given operator. 4. Otherwise
     * get {@link Number#intValue()} from both sides and compare using given operator. 3. If both sides are of the same
     * type and implement the {@link Comparable} interface then use {@code compareTo} method. 4. If none of the above
     * are met then a {@link ParseException} is thrown.
     *
     * @param left
     *            The token representing the left side of a comparison expression.
     * @param right
     *            The token representing the right side of a comparison expression.
     * @param op
     *            This is a representation of a comparison operator {@literal (eg. <, <=, >, >=, ==, !=) }
    ...
    public boolean evaluate(final Token<?> left, final Token<?> right, final StellarParser.ComparisonOpContext op) {
        if (left.getValue() == null || right.getValue() == null) {
            ...
            return compareNumbers((Number) left.getValue(), (Number) right.getValue(), op);
                ...
                && right.getValue() instanceof Comparable) {
            return compare((Comparable<?>) left.getValue(), (Comparable<?>) right.getValue(), op);
        }

        throw new ParseException("Unsupported operations. The following expression is invalid: " + left.getValue() + op
                + right.getValue());
    }

    /**
     * This method uses the inputs' ability to compare with one another's values by using the {@code compareTo} method.
     * It will use this and the operator to evaluate the output.
     *
     * @param l
     *            The value of the left side of the expression.
     * @param r
     ...
     * @return A boolean value representing the comparison of the two values with the given operator. For example,
     *         {@code 1 <= 1} would be true.
     */
